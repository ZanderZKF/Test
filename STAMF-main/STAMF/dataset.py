...
def load_list(dataset_list, data_root):
    ...
    for dataset_name in dataset_list:
        ...
        for depth in depth_files:
            if not depth.endswith('.png') and not depth.endswith('.jpg'):
                continue
            if len(depth) <= 4: # Skip files like '.png'
                continue
            images.append(depth_root.replace('/depth/', '/RGB/') + depth[:-4]+'.png')
            depths.append(depth_root + depth[:-4]+'.png')
            labels.append(depth_root.replace('/depth/', '/GT/') + depth[:-4]+'.png')
            edge_path = depth_root.replace('/depth/', '/edge/') + depth[:-4]+'_edge.png'
            if not os.path.exists(edge_path):
                edge_path = depth_root.replace('/depth/', '/GT/') + depth[:-4]+'.png'
            contours.append(edge_path)
...
class ImageData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            ...
            if '_edge.png' in self.contour_path[item]:
                contour = Image.open(self.contour_path[item]).convert('L')
            else:
                from PIL import ImageFilter
                contour = label.filter(ImageFilter.FIND_EDGES)
