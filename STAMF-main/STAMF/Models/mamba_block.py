    ...
    def forward_features(self, x1, x2=None, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.FlagForward == 0:
            ...
            if self.if_cls_token:
                if self.use_double_cls_token:#不会执行
                    cls_token_head = self.cls_token_head.expand(B, -1, -1)
                    cls_token_tail = self.cls_token_tail.expand(B, -1, -1)
                    token_position = [0, M + 1]
                    x1 = torch.cat((cls_token_head, x1, cls_token_tail), dim=1)
                    M = x1.shape[1]
                else:#执行这个
            ...
            if self.if_cls_token:#走这个
                if self.use_double_cls_token:#不走这个
                    return (hidden_states[:, token_position[0], :] + hidden_states[:, token_position[1], :]) / 2
                else:#走这个
                    if self.use_middle_cls_token:#走这个
                        return hidden_states[:, token_position, :]
                    elif if_random_cls_token_position:#不走
                        return hidden_states[:, token_position, :]
                    else:#不走
                        return hidden_states[:, token_position, :]
            ...
            elif self.final_pool_type == 'mean':#走这个
